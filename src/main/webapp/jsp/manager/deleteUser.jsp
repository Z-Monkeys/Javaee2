...
<html>
  <head>
  	...
  	function deleteUser(){
  		...
  		var id="";
  		for(var i=0;i<checkBoxs.length;i++){
  			if(id.length<1&&checkBoxs[i].checked==true){
  				id+=checkBoxs[i].value;
  			}else if(id.length>=1&&checkBoxs[i].checked==true){
  				id=id+","+checkBoxs[i].value;
  			}
  		}
  		if(id.length<1){
  			alert("请选择要删除的用户");
  ...
  <jsp:include page="/head2.jsp"></jsp:include>
    	<form action="/news/servlet/UserServlet?type1=deleteUser" id="myForm" method="post">
    		<table align="center" border="1">
    			<tr>
    				<td>
    					<input type="checkbox" name="checkBoxAll" onchange="checkAll(this);">
    				</td>
    				<td>
    			...
    			<c:forEach items="${users}" var="users">
    				<tr>
    					<td>
    						<input type="checkbox" name="checkBox" value="${users.userId}">
    					</td>
    		...
    		<table align="center" border="1">
    			<tr>
    				<td>
    					<a href="javascript:void(0);" onclick="deleteUser();">删除所选用户</a>
    				</td>
